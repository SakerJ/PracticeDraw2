...
public class Practice14MaskFilterView extends View {
    ...
    private MaskFilter mFilterNormal;
    private MaskFilter mFilterInner;
    private MaskFilter mFilterOuter;
    private MaskFilter mFilterSolid;

    {
        setLayerType(LAYER_TYPE_SOFTWARE, null);
        bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.what_the_fuck);

        mFilterNormal = new BlurMaskFilter(50, BlurMaskFilter.Blur.NORMAL);
        mFilterInner = new BlurMaskFilter(50, BlurMaskFilter.Blur.INNER);
        mFilterOuter = new BlurMaskFilter(50, BlurMaskFilter.Blur.OUTER);
        mFilterSolid = new BlurMaskFilter(50, BlurMaskFilter.Blur.SOLID);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 第一个：NORMAL
        paint.setMaskFilter(mFilterNormal);
        canvas.drawBitmap(bitmap, 100, 50, paint);

        // 第二个：INNER
        paint.setMaskFilter(mFilterInner);
        canvas.drawBitmap(bitmap, bitmap.getWidth() + 200, 50, paint);

        // 第三个：OUTER
        paint.setMaskFilter(mFilterOuter);
        canvas.drawBitmap(bitmap, 100, bitmap.getHeight() + 100, paint);

        // 第四个：SOLID
        paint.setMaskFilter(mFilterSolid);
